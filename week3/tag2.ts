//!--------Aufgabe 1 & 2---------
// Define a type "NameBirthday" which is an array of two values, first a string name and second a Date object containing the birthdate
type NameBirthday = [name: string, birthday: Date];
...
// Call the "daysUntil" function passing in an array of ["Kevin", new Date("1992-08-10")] as argument 
console.log("Aufgabe 2: ", daysUntil(["Kevin", new Date("1992-08-10")]));
...
type NameBirthday3 = [name: string, birthday: Date];

// Define a function "daysUntil" that takes in a "NameBirthday" parameter and returns a number specifying how many milliseconds are left until the next birthday
function daysUntil2(nameBirthday: NameBirthday3): number {
    // Array destructuring to extract the 'name' and 'birthday' variables from the tuple
    const [name, birthday] = nameBirthday;
...
const nameBirthdays: NameBirthday[] = [
    ["Kevin", new Date("1992-08-10")],
...
for (const [name, birthday] of nameBirthdays) {
    const msUntil = daysUntil([name, birthday]);
...
type NameBirthday2 = {
  ...
  function getNextBirthdayName(persons: NameBirthday2[]): string | null {
  ...
  console.log(`The next birthday is for ${nextBirthdayName}`);
